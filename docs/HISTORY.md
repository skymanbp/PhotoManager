...
- CI ✅ `.github/workflows/build.yml`（2026-08-28；DESIGN-P8 §26「CI」行）：windows-latest 单链——版本一致闸（package.yaml / Cargo.toml / Cargo.lock / tauri.conf.json / DESIGN-COMMANDS 状态行 / tag 名）→ `stack test` → `pm --version` 闸 → sidecar → `@tauri-apps/cli@2.11.4` build（remap 工作区 + 用户目录）→ `scripts/leakscan.py` → zip + NSIS + `sha256.txt` 同 run → artifact；tag `v*` 触发 release job（`softprops/action-gh-release`，说明附 SHA-256 + `docs/release-notes/v<版本>.md`）。抓包分支 `ci-probe` 首跑 409/418：提权 runner + MSYS2 bash 继承已启用的 SeBackup/SeRestore 特权，ACL 注入用例假红 → 套件启动自禁两项特权（cbits `pm_disable_backup_privileges`）；重跑 run 33152288443 418/418
- 复核 ✅ 用户 GUI 复核 + 首次真实数据跑（2026-08-28；REVIEW-LOG「用户复核 + 首次真实数据跑」）：复核两项（侧栏脚注溢出、同行按钮不齐）→ 58f136d 只改 CSS；真实数据按用户裁定处理——不动相册、vault 15 张保持暂不同步、成片两处相机计数回绕的同名照片改名、退役旧编辑 tif 挪进待修改（全部 mv，sha 前后一致，零删除）；pm scan 4633 一致、doctor 无 Bad、album candidates 同名 ⚠ 0 · 非 jpg 0；418/418
- 1.0.0 ✅ 收官（2026-08-28）：版本串五处 bump（package.yaml / Cargo.toml / Cargo.lock / tauri.conf.json / DESIGN-COMMANDS 状态行，`pm --version` 走 Cabal 宏）、README 路线图、`docs/release-notes/v1.0.0.md`；tag `v1.0.0` 由 CI release job 出 zip + NSIS + SHA-256（本机不再编发布二进制）；README 改英文主入口 + `README.zh.md` 中文镜像（DocDrift 哨兵两份都盯）；418/418，警告 0
- 计划页/忽略候选 ✅ 增补（2026-08-31；用户裁定：标注+删除+一键清理 / 按内容 sha 忽略 / 本地 build 先用；DESIGN-COMMANDS「计划页完善 + 候选忽略」节）：① `pm plan [list]|rm|prune`——执行态从 journal 折叠（`Pm.Plan.planExecs`：普通 Done 计入、`~r` 复位剔除、`~d` 位移不计、重试成功抹失败；`planExecuted` 保守判据：草稿不算、待裁决残余不算），计划文件永不回写；`deletePlan` 走 `isValidPlanId` + `requirePmTrusted` + 句柄删；prune 只删已执行且可再生成的计划文件，journal/undo 不动；② `pm album ignore|unignore`——按内容 sha 记 `.pm/album-ignore.json`（`requireMain` + `withRootLock` 事务、fail-closed 全错一次列完），改名/挪夹后仍生效、重新导出的新字节重新出现，候选清单分列已忽略 + 失效提示；③ serve 写端点九 → 十二（`/api/album/ignore`、`/api/plan/delete`、`/api/plans/prune`，`--writable` 级），GET `/api/plans` 附 done/failed/executed/lastRunAt；④ GUI 计划页执行态标注/已执行行淡化/逐行两击删除/「清理已执行」，归档页候选卡「忽略」+ 折叠区取消/清失效；⑤ `test/PlanExecTests.hs` 3 例 + AlbumTests/ServeP8Tests 新 5 例，DocDrift 哨兵（路由名册/README 计数）随改；426/426，警告 0
- 备份范围 ✅ 主库 − 暂存区（2026-08-31 用户裁定：三仓思路对齐——`To-Be-Sync'd\` 只是中转，不进备份盘）：收窄在 `Pm.Diff.backupDiff` 单点（主库侧先滤掉 stagingTop，BackupCmd 比对 / apply 后缓存重算 / status 卡全部继承）；暂存文件不再产生 add/update，备份盘上已有的暂存副本按不在范围计入 EXTRA（只报告，删除由人做）；清暂存三副本屏障不受影响（`planClean` 按 sha 认归档层的备份见证，Clean.hs `backupBySha`）；PlannerTests 新增范围用例 + 突变（去掉过滤即转红）；427/427，警告 0
- 1.1.0 ✅ 发布（2026-08-31，用户令「pm收口。提交+推送并发布新release」）：版本串五处 bump（package.yaml / Cargo.toml / Cargo.lock / tauri.conf.json / DESIGN-COMMANDS 状态行）、`docs/release-notes/v1.1.0.md`、REVIEW-LOG「1.1.0 增补批」节（第一方全量自审：0 critical/0 major/1 minor 登记；突变 m1–m7 判红记录；真实库落地复核）；tag `v1.1.0` 由 CI release job 出 zip + NSIS + SHA-256（本机不编发布二进制）；本机装机随 release 资产更新；427/427，警告 0
- 未来 mtime 复用 ✅ 根修（2026-09-02，备份盘接入后的真实盘复核；用户令「对照PM当前版本，检查问题」）：`Pm.Hash.statHitStable` 只认「hash 时刻晚于 mtime 2 s 以上」，mtime 在未来的文件（相机时钟错 + 拷贝保留时间戳——真实库 `Raw\2023\23-06-Cornwall-Raw` 120 张 + `23-07-Wales-Derbyshire-Scotland-Raw` 2 张 ARW 的时钟在 2027 年）永远判 racy，主库与备份盘每次 scan 各白 hash 14 GiB；判据加「当前时刻」参数——危险的只是 mtime 起 2 s 的写入窗口，窗口尚未到来同样可信，窗口内 / 窗口过后但 hash 仍早于 mtime 不信任（到期重 hash 一次即永久回稳），时钟回拨本就不在威胁模型内；scan 复用 / vault `shaViaCache` / 夹具 `plantStaleCatalog` 三处调用同改；`caseFutureMtimeReuse` 6 断言 + 突变（去掉第二分支 → 红）。同批真实盘动作（REVIEW-LOG「1.1.0 后真实盘复核」节）：备份盘 EXTRA 270（23.7 GiB，含暂存副本 245）移入回收站、备份计划 3 项落位、二次 `pm backup` 新增 0 · 更新 0 · 一致 4611 · EXTRA 0；428/428，警告 0
- 1.1.1 ✅ 发布（2026-09-02，用户 AskUserQuestion 裁定「发布 1.1.1」）：版本串五处 bump（package.yaml / Cargo.toml / Cargo.lock / tauri.conf.json / DESIGN-COMMANDS 状态行）、`docs/release-notes/v1.1.1.md`、README 路线图行；内容 = 未来 mtime 复用根修（同日上一行）+ 真实盘复核登记；tag `v1.1.1` 由 CI release job 出 zip + NSIS + SHA-256，本机装机随 release 资产更新。同日用户裁定的三处 Raw 事件夹改名（RAW-2025-Summer-Providence → 25-08-Providence-Raw、RAW-2025-Autumn-Providence → 25-11-Providence-Raw、RAW-2025-Summer-Atlanta → 25-08-Atlanta-Raw）在主库与备份盘两侧同步 mv：`pm scan` 4633（挪位 59 重 hash 一次）、`pm names` 合规 40 · 待裁决 0 · 无法识别 2、`pm backup` 一致 4611 · EXTRA 0；EXIF 假时钟按裁定不动、登记为已知；428/428，警告 0
- 真实盘第三批（2026-09-02 下午，用户 AskUserQuestion 三/四批裁定；REVIEW-LOG「1.1.0 后真实盘复核」节续）：EXIF 假时钟改正（ExifTool 13.59，491 图 + 27 xmp 纯天数位移，NTFS 创建时间对齐；用户改令「月份正确就行」推翻上一行的「不动」）、`&` 双月夹按拍摄月拆夹 + 五处夹名月份改名（Portsmouth 23-04→23-05、Anhui 23-11→23-10 含成片 `安徽`、Providence 秋 25-11→25-10 含成片、Atlanta 25-08→25-07、Tennessee 25-08→25-07 含成片；`25-08-PR-Raw` 改正后 EXIF 显示行程 2025-08-20..26，本就在 8 月，保持原名）两盘同步，`pm names` 合规 44 · 待裁决 0 · 无法识别 0；`pm scan` 重 hash 532；vault DRIFT 10 → push / resolve --keep src / apply 覆盖 + 4 条 HELD 重标（vault 仓 10 modified 随后按裁定提交推送，commit 3bb1f93，见下一行）；备份盘 527 组 supersede 更新——外置盘当日瞬断 8 次，裸跑 `pm apply` 两次被打断后改用新脚本 `scripts/backup_watchdog.py`（分块 `--only` 续跑、等盘自动续、认「Done 丢失」洞、收尾 doctor --repair）跑完，最终 `pm backup` 新增 0 · 更新 0 · 一致 4611 · EXTRA 0，527 目标全文重读 sha 527/527 相符（`scripts/verify_backup_dst.py`）；README 两语各加看门狗一行；pm 代码零改动，428/428，警告 0
- 收口清理与瞬断核验内核（2026-09-02 傍晚，用户 AskUserQuestion 第五/六批裁定；REVIEW-LOG「清理与上线实录」）：E: 回收站 270 项永久删除，三处 `.pm/trash` 清空（备份盘 529 / vault 10 / 主库 231，1 项三副本不过被 HELD）；vault 仓 10 modified 按裁定提交推送 3bb1f93；备份盘 08-26 写入后从未回读的 588 条全文重读 588/588 相符（`scripts/verify_backup_entries.py`，1 次瞬断重挂由 `--retry` 补读），`pm doctor --deep --backup` 按裁定不跑；三支盘上脚本共用新内核 `scripts/backup_verify.py`（等盘回来、冷却、任何读错先重排队，watchdog 改用同一 Drive）；文档漂移审计 18 条确认全部改入（README ×2 / DESIGN / DESIGN-COMMANDS / HISTORY / REVIEW-LOG / 手册第 1/2/3/7 章）；README 两语各加两支核验一行；REVIEW-LOG 行尾统一 LF；pm 代码零改动，428/428，警告 0
- 1.1.2 ✅ 瞬断保护内建（2026-09-02 晚，用户裁定「防瞬断功能加入正式功能，防止备份/检查时硬盘断连；版本升到 1.1.2，发布新 release；更新全量文档与本机安装版本」；REVIEW-LOG「1.1.2 瞬断保护内建」节）：新模块 `Pm.Removable`（盘在 = root-id.json 可读；`IOException` 三分：确定性原样抛 / 盘不在等它回来 / 盘在 EINVAL 短停；`withDriveRetry` / `ensureDrive` / `requireDrive` / `scanRootRetry` 按 pass 复用重扫 / `execPlanRetry` 会话级按组续跑 + 两场之间 `doctor --repair` 自愈 + 按 journal 结算）；`ExecEnv.eeProgress` 逐项进度钩子（内核唯一改动一行）；`Pm.Doctor.runDoctorWith`（整场幂等重跑、`--deep` 逐条等盘、场末 `requireDrive`）；`Pm.Derived` 从 Convert 拆出（打断 Doctor→Convert→Cli 的依赖环，字节级搬移）；配置 `[backup] drive-wait`（缺省 1800，0 = 关）贯通 TOML / `pm config set --drive-wait` / `POST /api/config driveWait` / GUI 设置页备份卡；`Pm.Cli.executePlanNowWith`、`Pm.BackupCmd.runBackupDiff`、`pm doctor` 三处接线；`scripts/backup_watchdog.py` 退役（两支核验脚本保留）；新增 `RemovableTests` 8 例（盘的替身 = 挪走 root-id.json，插回由打印口触发；每个「开」配「关」对偶）+ 配置三处用例；测试夹具 `Config` 位置构造 24 处补 `(Just 0)`；版本串五处 bump；README ×2 / DESIGN §6.4 §14 / DESIGN-COMMANDS §9 + 状态行 / DESIGN-GUI §11 / release notes / 手册 v7 同步；436/436，警告 0
- 1.1.3 ✅ 计划页失效草稿 + 版式（2026-09-02 深夜，用户反馈「为什么还有那些好多条未执行计划」「已执行计划在 GUI 页面上看着很乱，各种溢出文字重叠」并令「修复问题，更新版本。全量更新文档、readme、本机安装版本，然后发布新 release」；REVIEW-LOG「1.1.3 计划页失效草稿 + 版式」节）：`Pm.Plan.planStale`（从未执行 + 有待办 + 每条待办的源都已不在而卷还在 = 失效；`opSource` 给三种 Op 的源；探测抛出按源在计；journal 有告警不判）+ `runTag` 首参 stale 给「已失效（源已不在）」；`prunePlans` = 已执行 ∪ 失效（journal 有告警的根整根不删）；`pm plan list` 显示失效；serve `GET /api/plans` 加 `stale` / `state`（GUI 原样显示服务端措辞，修掉「已执行（余 8 项待裁决）」被显示成「部分 8/8」）；GUI 计划页上下整宽堆叠（`.split` 单栏 + `.tbl-scroll` ≤ 42 vh 自滚、表头钉住；根因：两栏各半 + 9 列 nowrap + `.table overflow:hidden` 让列表溢到明细面板下面）、明细路径列任意断行 / 待裁决原因另起一行 / 失效草稿黄横幅且不渲染「执行」、按钮改「清理已执行/失效」；`PlanExecTests` 加 1 例（源在 / 只缺一条 / 全缺 / 有 Done / 仅跳过 / 卷不在 / prune 清失效留活草稿）；版本串五处 bump；README ×2 / DESIGN-COMMANDS 新节 + 状态行 / DESIGN-GUI §11 / release notes / 两本手册同步；437/437，警告 0
- 1.2.0 ✅ I7 判定侧（2026-09-03，用户裁定「实现 §10.3 第 2 项的消费侧」；REVIEW-LOG「1.2.0 I7 判定侧」节）：`Pm.Doctor.i7Findings`——相册 ⊆ 成片 ∪ inbox-origin 的**判定侧**（记录侧 P6-D 就位后一直没人读它）：已解释 = 索引里有同 sha 的成片副本，或 journal 里有 dst 恰是这条相册路径、sha 与盘面相符、src 经 `pathAtOrUnder` 明确解析在**库外**的 Copy 记录（`_inbox` 源事后移进 `_done` 不改判——证据是记录）；两条都不成立逐条 `I7` Warn 交人裁决，收尾一条 Info 汇总；`--repair` 的白名单不含它；journal 有告警或快照坏代回退 → 整条不判（判据含否定式，fail-closed）。层名 `相册`/`成片` 收进 `Pm.Types` 单一定义（`Pm.Album` 再导出；`Pm.Doctor` import `Pm.Album` 会与 `Pm.Cli` 成环）；`SweepTests` 的 `--deep` 汇报夹具两张照片改放成片层（相册层无源照片按新判据正是违例）。IngestTests 新增 3 例、突变 m1–m5 各判红一例；真实库实测「94 张 = 成片副本 94 · inbox 来源 0 · 未解释 0」exit 0；版本串五处 bump（package.yaml / Cargo.toml / Cargo.lock / tauri.conf.json / DESIGN-COMMANDS 状态行）+ README ×2 / DESIGN §2 §5 §14 / DESIGN-COMMANDS §10.3 + 新节 / release notes / specs 快照如期标注 / 两本手册按 1.2.0 重生成（操作手册 v9 71 页九处改、GUI 图册 16 页只改版本行；PDF 仍不入库）；440/440，警告 0
