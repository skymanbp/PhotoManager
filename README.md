...
> **三十五轮**，见 [docs/REVIEW-LOG.md](docs/REVIEW-LOG.md)），每道闸都配"删掉
> 它就转红"的突变验证用例（308 例，GHC 警告 0）。
...
6. **三十五轮对抗评审门禁 + 突变验证**。每条写路径合并前过独立评审（NO-GO 逐条
...
| 测试套件（308 例，整套序列化跑——进程级 stdout 重定向所需） | 10–40 s | `stack test` |
| GHC 警告 | 0 | `stack build` |
| 对抗评审门禁 | 35 轮（NO-GO 的每条 finding 逐条第一方核实后处置） | [REVIEW-LOG](docs/REVIEW-LOG.md) |
| 突变验证 | 每道承重闸一个突变、配对用例转红（近三轮 2+6+3 道全数通过） | REVIEW-LOG 各轮收敛证据 |
...
- ~~`pm vault ingest`~~ ✅ P6-D：pm 只拷 root 内（相册 + vault 类目两份计划），
  ...
  已全修（执行次序闸收紧，见 REVIEW-LOG；33–35 轮连续收口「读口
  fail-closed」漏网，35 轮按 IO 读原语全集清点扫尽），真实 `_inbox` 首次使用仍在门禁收敛确认 + 用户裁定之后。
