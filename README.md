...
- P5-A ✅ `pm sort`（241 例，GHC 警告 0）：散落新照片（相机卡/下载目录）
  按 EXIF 拍摄时间分段 → 暂存区
  ...
  **一轮对抗审查（4 视角 + 逐条独立反驳）改掉 10 条真缺陷**，按根因归六类一次
  ...
  读取失败整批拒绝。此后 codex 第 25 轮门禁 NO-GO，6 条逐条核实**全部成立**，按四个根因各改一处（IFD 偏移下界、扩展名清单唯一化、跳过前复核目标实际内容、遍历复用 listTree），并让每个没进计划的源文件都被列出来。二十道闸各自突变转红（纯核心 8 + IO 层 5 + 本轮 7）。经过见 REVIEW-LOG。
