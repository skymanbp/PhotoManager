...
> **三十六轮**，见 [docs/REVIEW-LOG.md](docs/REVIEW-LOG.md)），每道闸都配"删掉
...
6. **三十六轮对抗评审门禁 + 突变验证**。每条写路径合并前过独立评审（NO-GO 逐条
...
| 对抗评审门禁 | 36 轮（NO-GO 的每条 finding 逐条第一方核实后处置） | [REVIEW-LOG](docs/REVIEW-LOG.md) |
| 突变验证 | 每道承重闸一个突变、配对用例转红（近三轮 6+3+2 道全数通过） | REVIEW-LOG 各轮收敛证据 |
...
- ~~`pm vault ingest`~~ ✅ P6-D：pm 只拷 root 内（相册 + vault 类目两份计划），
  ...
  已全修（执行次序闸收紧，见 REVIEW-LOG；33–36 轮连续收口「读口
  fail-closed」漏网，35 轮按 IO 读原语全集清点扫尽、36 轮又关掉 I11 存在性布尔探针把「查不出」塌成「不存在」的放行口），真实 `_inbox` 首次使用仍在门禁收敛确认 + 用户裁定之后。
