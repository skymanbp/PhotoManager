...
> 验证用例（440 例，GHC 警告 0）；没有落点的（GUI 无 harness、并发交错无确定
...
- **功能九 · 安全网**：`pm trash`（隔离区列表/清空——待删条目一律逐项列出、加
  ...
  的那批」，`--deep` 重读重 hash **全库**索引条目并汇报条目数与不符数；`I7` 行另校验
  拓扑不变量 相册 ⊆ 成片 ∪ inbox-origin——相册里既没有同 sha 的成片副本、journal 里
...
计划」把三件事一起做）、**计划**（逐项明细 + 执行态标注——已执行/部分/未执行/已失效从 journal
折叠而来（措辞就是服务端 `pm plan list` 那一句），已执行与已失效的行淡化——失效 = 从未执行、
而每条待办的源都已不在盘上的草稿（被后来的计划顶掉了）；1.1.3 起列表与明细上下整宽堆叠；
...
pm plan [list]                   # 列出计划与执行态（已执行/部分/未执行/已失效——从 journal 折叠 + 探源，计划文件不回写）
pm plan rm <id> … / pm plan prune   # 删除计划文件 / 一键清理已执行与失效草稿（从未执行且每条待办的源都已不在；只删可再生成的文件，journal/undo 不受影响）
...
| 测试套件（440 例，整套序列化跑——进程级 stdout 重定向所需） | 10–90 s | `stack test` |
...
- ~~计划页失效草稿 + 版式~~ ✅ 1.1.3（用户反馈 2026-09-02「为什么还有那么多未执行计划」+
  已执行计划的明细文字溢出重叠）：`Pm.Plan.planStale`——从未执行、每条待办的源（拷贝源 /
  改名旧路径 / 隔离 victim）都已不在盘上而卷还在的草稿 = **失效**；`pm plan list`、
  `GET /api/plans`（新增 `stale` + `state`）、GUI 都标「已失效（源已不在）」，`pm plan prune` /
  「清理已执行/失效」连它一起清（卷不在 = 拔掉的相机卡不算；journal 有告警的根不判、不删，
  fail-closed）。GUI 执行态改为原样显示服务端措辞（此前把「已执行（余 8 项待裁决）」显示成
  「部分 8/8」）；计划页改上下整宽堆叠（两栏各半时 9 列 nowrap 的列表被明细面板盖掉尾列、
  路径逐字折行）。
- ~~I7 的消费侧（相册 ⊆ 成片 ∪ inbox-origin）~~ ✅ 1.2.0（用户裁定 2026-09-03）：记录侧
  自 P6-D 就位（`pm vault ingest` 在 journal 里写下带**库外** `srcAbs` 的 Copy Intent），
  但一直没人读它。`pm doctor` 现在多一条 `I7` 行：相册里的一张照片，成片里有同 sha 的
  副本、或 journal 里有一条 dst 恰是这条相册路径、sha 与盘上现字节相同、src 解析在库外的
  Copy 记录，即为**已解释**（源后来被移进 `_done` 不改判——证据是记录不是源文件）；两条都
  不成立的逐条报 Warn 交人裁决。`--repair` 不碰它；journal 或快照读出告警时整条判据不判
  （判据里有「journal 里没有别的来源记录」这条否定式，折叠不全时不可信）。
