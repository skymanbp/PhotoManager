...
main = do
  ...
  withSystemTempDirectory "pm-test-cfg" $ \d -> do
    ...
    defaultMain
      (localOption (NumThreads 1) $ testGroup "pm" [kernelTests, plannerTests, vaultTests, vaultHoldTests, namesTests, guardTests, pathGuardTests, stateGuardTests, serveTests, sortTests, dedupeTests, ingestTests, scanGuardTests])
